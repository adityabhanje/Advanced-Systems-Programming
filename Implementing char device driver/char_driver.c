...
loff_t dev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos > dev->buffer_size) {
                temp_buffer = (unsigned char*)kmalloc(newpos, GFP_KERNEL);
                memset(temp_buffer, 0, newpos);
                memcpy(temp_buffer, dev->ramDisk, dev->buffer_size);
                kfree(dev->ramDisk);
                dev->ramDisk = temp_buffer;
...
static int controldev(struct ASP_mycdrv *dev, int minor, 
        struct class *class)
{
        int err = 0;
...
static int __init my_init(void)
{
	...
	for (i = 0; i < NUM_DEVICES; ++i) 
        {     
		mycdev_device = (struct ASP_mycdrv *)kmalloc(sizeof(struct ASP_mycdrv), 
						GFP_KERNEL);
